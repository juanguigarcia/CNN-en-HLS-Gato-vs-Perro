            ...
            for (int pi = 0; pi < pool_size; ++pi) {
				...
				#pragma HLS UNROLL factor=2
                    ...
                    if (x < image_size && y < image_size) {
                        max_val = fmax(max_val, *(input+x*image_size+y));
                    }
                }
            }
...
#pragma HLS INTERFACE s_axilite port=input_image bundle=CONTROL_BUS
	...
	#pragma HLS INTERFACE m_axi port=conv1_output depth=1024 offset=slave bundle=DDR
	#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=conv1_output

    float pool1_output[IMG_SIZE / 2][IMG_SIZE / 2];
	#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=pool1_output
	#pragma HLS INTERFACE m_axi port=pool1_output depth=1024 offset=slave bundle=DDR

    float conv2_output[IMG_SIZE / 2][IMG_SIZE / 2];
	#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=conv2_output
	#pragma HLS INTERFACE m_axi port=conv2_output depth=1024 offset=slave bundle=DDR

    float pool2_output[IMG_SIZE / 4][IMG_SIZE / 4];
//#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=pool2_output
	#pragma HLS INTERFACE m_axi port=pool2_output depth=1024 offset=slave bundle=DDR

    float conv3_output[IMG_SIZE / 4][IMG_SIZE / 4];
//#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=conv3_output
	#pragma HLS INTERFACE m_axi port=conv3_output depth=1024 offset=slave bundle=DDR

    float pool3_output[IMG_SIZE / 8][IMG_SIZE / 8];
//#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=pool3_output
	#pragma HLS INTERFACE m_axi port=pool3_output depth=1024 offset=slave bundle=DDR

    float flatten_output[IMG_SIZE * IMG_SIZE / 64];
//#pragma HLS ARRAY_PARTITION dim=1 factor=2 type=cyclic variable=flatten_output
